from brownie import DutchToken, TokenFarm, network, config
...
def deploy_token_farm_and_dutch_token(update_front_end_flag=False):
    account = get_account()
    dutch_token = DutchToken.deploy(KEPT_BALANCE, {"from": account})
    token_farm = TokenFarm.deploy(
        dutch_token.address,
        {"from": account},
        publish_source=config["networks"][network.show_active()]["verify"],
    )
    tx = dutch_token.transfer(
        token_farm.address,
        dutch_token.totalSupply() - KEPT_BALANCE,
        {"from": account},
    )
    tx.wait(1)
    ...
    add_allowed_tokens(
        token_farm,
        {
            dutch_token: get_contract("dai_usd_price_feed"),
...
def main():
    deploy_token_farm_and_dutch_token(update_front_end_flag=True)
